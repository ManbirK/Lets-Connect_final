  ...
  firebase.database().ref("/" + room_name).on('value', function (snapshot) {
        ...
        snapshot.forEach(function (childSnapshot) {
              ...
              if (childKey != "purpose") {
                    ...
                    //Start code
                    new_name= message_data['username'];
                    new_message = message_data['message'];
                    new_likes = message_data['likes'];

                    name_tag = "<h3> " + new_name + " </h3> <br>";

                    message_tag = "<h4>" + new_message + "</h4> <br>";

                    like_button = "<button class = 'btn btn-primary'  id = " + firebase_message_id + " value = " + new_likes + " onclick = 'add_likes(this.id)'><span class = 'glyphicon glyphicon-thumbs-up'> </span> Likes : "+ new_likes + " </button> <hr>";
